...
epic::index::PublicHelpTheta::PublicHelpTheta(Game* g)
	: RawPublicHelpThetaSD(g) {}

std::vector<epic::bigFloat> epic::index::PublicHelpTheta::calculate(Game* g) {
	...
	std::vector<bigFloat> solution(g->getNumberOfPlayers());
	{
		if (log::out.getLogLevel() <= log::info) {
			...
			for (longUInt i = 0; i < g->getNumberOfPlayers(); ++i) {
				...
				log::out << "Player " << g->playerIndexToNumber(i) << ": " << output.get_str() << log::endl;
			}
...
std::string epic::index::PublicHelpTheta::getFullName() {
	return "PublicHelpTheta (with Dubey-Shapley identity)";
}
